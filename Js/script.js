...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Esperemos tu cumpleaños juntos mamotica con mucho amor de tu goldo `;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "con amor del goldo para la mamotica  ";
...
// Reloj cuenta regresiva para el 25/08/2025
function showCountdownClock() {
  ...
  if (!clock) {
    ...
    clock.style.fontSize = '2.2rem';
    clock.style.fontWeight = 'bold';
    clock.style.padding = '16px 28px';
    clock.style.borderRadius = '18px';
    clock.style.boxShadow = '0 2px 16px #ff4da6aa';
    clock.style.zIndex = 100;
    clock.style.letterSpacing = '2px';
    clock.style.textAlign = 'center';
    document.body.appendChild(clock);
  }
  const target = new Date('2025-08-25T00:00:00');
  function updateClock() {
    const now = new Date();
    const diff = target - now;
    if (diff <= 0) {
      clock.textContent = "¡Llegó el día!";
      return;
    }
    ...
    clock.innerHTML = 
      `<span style="color:#fff">${days}</span>d ` +
      `<span style="color:#fff">${hours}</span>h ` +
      `<span style="color:#fff">${minutes}</span>m ` +
      `<span style="color:#fff">${seconds}</span>s<br>` +
      `<span style="font-size:1.1rem;font-weight:normal;">para el 25 de agosto de 2025</span>`;
  }
